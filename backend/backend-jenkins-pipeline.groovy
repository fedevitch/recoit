  ...
  stages {        
    ...
    stage('Build and Push to registry') {
      steps {
        container('buildah') {
            sh '''
            buildah bud $(xargs -a .env.staging -I{} echo --build-arg {}) --format=docker -t $REGISTRY/$IMAGE_NAME:$BUILD_NUMBER -t $REGISTRY/$IMAGE_NAME:latest -t $REGISTRY/$IMAGE_NAME:staging .
    ...
    stage('Deploy App to Kubernetes') { 
      steps {
        container('kubectl') {
          withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'recoit-k8s-secret', namespace: 'recoit', restrictKubeConfigAccess: false, serverUrl: 'PUT SERVER URL WITH YOUR K8S SERVER') {
            sh 'kubectl rollout restart -n recoit deployment recoit-backend-deployment'
          }
        }  
      }
    }
